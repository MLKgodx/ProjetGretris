...
// Vérifier que l'utilisateur est connecté et accède à ses propres données
if (!isset($_SESSION['PlayerID']) || $_SESSION['PlayerID'] != $playerInfo['PlayerID']) {
    // Log de l'incident de sécurité
    error_log("Tentative d'accès non autorisé dans la vue: Session ID " . 
              ($_SESSION['PlayerID'] ?? 'non défini') . 
              " tente d'accéder au compte " . 
              ($playerInfo['PlayerID'] ?? 'non défini'));
