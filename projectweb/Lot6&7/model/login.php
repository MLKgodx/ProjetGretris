...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user && password_verify($inputPassword, $user['Password'])) {
        if ($user['TimeOut']) {
            $invalidlogin = "Votre compte est verrouillé en raison de plusieurs tentatives de connexion échouées.";
        } else {
        ...
        if ($user) {
            ...
            if ($user['LoginAttemps'] + 1 >= 3) {
                ...
                $_SESSION['errorMessage'] = "Votre compte a été verrouillé en raison de plusieurs tentatives de connexion échouées.";
            } else {
                $_SESSION['invalidInfos'] = true;
